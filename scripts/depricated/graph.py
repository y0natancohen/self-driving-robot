...
def generate_graph():
    ...
    # add_edge('s1_w', 'sw_in', g, pi/2)

    add_edge('s4', 'e4', g, pi/2, line)  # because e4 == e4_stop_line
    # add_edge('s4', 'se_out', g, pi/2)

    # add_edge('e3_s', 'se_in', g, pi)
    add_edge('e3_s', 'e3_s_help', g, 3*pi/2 + help_delta_theta, amcl)
    add_edge('e3_s_help', 's2', g, pi, line)
    add_edge('e3_s_help', 's2_n', g, pi/2, line)

    # center edges
    add_edge('n3_s', 'n3_s_help', g, 3*pi/2 + help_delta_theta, amcl)
    add_edge('n3_s_help', 'c1_stop_line', g, 3*pi/2, line)
    ...
    add_edge('c1_w_help', 'w2_stop_line', g, pi, line)
    ...
    add_edge('c2_n_help', 'n4_stop_line', g, pi/2, line)
    ...
    add_edge('e1_w_help', 'c2_stop_line', g, pi, line)
    ...
    add_edge('w4_e_help', 'c3_stop_line', g, 0, line)
    ...
    add_edge('c3_s_help', 's1_stop_line', g, 3*pi/2, line)
...
def add_complete_4_group(p1s, p2s, p3s, p4s, g, n_is_out=False, s_is_out=False, e_is_out=False, w_is_out=False):
            ...
            for p3 in p3s:
                for p4 in p4s:
                    ...
                    if n_is_out:
                        ...
                        add_edge(p2, p1, g, pi, amcl)

                    if w_is_out:
                        add_edge(p1, p3, g, 3*pi/2, line)
                    else:
                    ...
                    # add_edge(p2, p4, g, 3*pi/2)
                    if e_is_out:
                        add_edge(p4, p2, g, pi/2, line)
                    else:
                        add_edge(p4, p2, g, pi/2, amcl)

                    if s_is_out:
                        add_edge(p3, p4, g, 0, line)
                    else:
